  ...
  //CustomerDAO객체의 getList 메소드 실행: 게시물 전체 목록을 가져온 후 request 객체에 넣어준다.
    public String getList(HttpServletRequest request) {
	    ...
	    try {
	    	...
	    	e.printStackTrace();
	        request.setAttribute("error", "고객목록을 정상적으로 가져오지 못했습니다!");
	    }
	    return "index.jsp";
	}
 	 	...
 	 	public String insertCustomer(HttpServletRequest request) {
 			...
 			try {
 				...
 				try {
 					...
 					String encodeName = URLEncoder.encode("고객정보가 정상적으로 등록되지 않았습니다.", "UTF-8");
 	 	...
 	 	public String updateCustomer(HttpServletRequest request) {
 	 		...
 	 		try {
 				...
 				dao.updateCustomer(b);
 	 			...
 	 			try {
 					//쿼리스트링의 한글깨짐을 방지하기 위해 UTF-8로 인코딩
 					String encodeName = URLEncoder.encode("고객정보가 정상적으로 수정되지 않았습니다.", "UTF-8");
